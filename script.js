    ...
    function fetchMovies() {
        return $.ajax({
            ...
            data: {
                ...
                query: query,
                language: selectedLanguage,
                page: 1 // Запрашиваем только первую страницу
            },
            success: function (data) {
                totalSearchResults = data.total_results;
                currentMovies = data.results;

                // Display all movies immediately
                displayMovies(currentMovies, '#moviesList', false);
            },
            error: function (error) {
        ...
        $.get(`${baseUrl}/movie/${movieId}`, {
            ...
            .done(function (movie) {
                $.get(`${baseUrl}/movie/${movieId}/videos`, {
                    ...
                    .done(function (data) {
                        console.log('Trailer data:', data); // Log the data to the console
                        ...
                        if (data.results.length > 0) {
                            ...
                            const trailerIframe = $('<iframe>').attr('src', `https://www.youtube.com/embed/${trailerKey}`).attr('allowfullscreen', true).css({
                                'width': '800px',   // Set the width of the trailer
                                'height': '500px',  // Set the height of the trailer
                                'min-width': '800px', // Set the minimum width of the trailer
                                'min-height': '500px' // Set the minimum height of the trailer
                            });
